 ...
 */
  ...
  static void print_vsense(TMC &st) { SERIAL_ECHOPGM_P(st.vsense() ? PSTR("1=.18") : PSTR("0=.325")); }
    ...
    static void _tmc_parse_drv_status(TMC2208Stepper &st, const TMC_drv_status_enum i) {
      ...
      void anker_tmc_get_result(uint8_t idex)
        {
           if(idex==1)
           {
             SERIAL_ECHO("X-result:");
             _tmc_parse_drv_status(stepperX,TMC_SG_RESULT);
             SERIAL_ECHO("\r\n");
           }
           else if(idex==2)
           {
             SERIAL_ECHO("Y-result:");
             _tmc_parse_drv_status(stepperY,TMC_SG_RESULT);
             SERIAL_ECHO("\r\n");
           }
        } 
      #endif
  ...
  void tmc_disable_stallguard(TMC2660Stepper, const bool) {};
   ...
   #if EITHER(ANKER_TMC_SET, USE_Z_SENSORLESS) 
