...
// Extruder offsets
  ...
  void reset_hotend_offsets() {
    constexpr float tmp[XYZA][HOTENDS] = { HOTEND_OFFSET_X, HOTEND_OFFSET_Y, HOTEND_OFFSET_Z, HOTEND_OFFSET_I};
...
void do_blocking_move_to(LINEAR_AXIS_ARGS(const float), const_feedRate_t fr_mm_s/*=0.0f*/) {
  ...
  const feedRate_t xy_feedrate = fr_mm_s ?: feedRate_t(XY_PROBE_FEEDRATE_MM_S);
    ...
    if(anker_z_offset.cs1237_start_convert)
    {
      ...
      // anker_z_offset.cs1237_enable();
      // cs1237_init_value();
      planner.anker_z_offset_synchronize();
    }
    else
    {
       SERIAL_ECHO(" cs1237_disable\r\n");
...
void do_blocking_move_to_x(const_float_t rx, const_feedRate_t fr_mm_s/*=0.0*/) {
  ...
  void do_z_clearance(const_float_t zclear, const bool lower_allowed/*=false*/) {
    ...
    if (!lower_allowed) NOLESS(zdest, current_position.z);
     #if ENABLED(ANKER_PROBE_SET)
       do_blocking_move_to_z(_MIN(zdest, Z_MAX_POS), MMM_TO_MMS(HOMING_RISE_SPEED));
     #else
...
void prepare_line_to_destination() {
  ...
  void do_homing_move(const AxisEnum axis, const float distance, const feedRate_t fr_mm_s=0.0, const bool final_approach=true) {
    ...
    if (is_home_dir) {
      ...
      TERN_(ANKER_SIMPLE_HOMING, if(axis == Z_AXIS){endstops.set_homing_filter(200);}else{endstops.set_homing_filter(50);}); // Z_filter = 200ms(2.5mm/s=2mm),X/Y_filter = 50ms(40mm/s=2mm)
    }
  ...
  bool anker_do_probe_homing_move(const AxisEnum axis, const float distance, const feedRate_t fr_mm_s=0.0, const bool final_approach=true) {
    ...
    if (is_home_dir) {
        ...
        if (axis == Z_AXIS && final_approach){
          ...
          #elif ANKER_PROBE_SET
            safe_delay(50); //Wait for the sensor signal to recover, at least 10ms.
          #endif
        }
      #endif
  ...
  void another_do_homing_move(const AxisEnum axis, const float distance, const feedRate_t fr_mm_s=0.0) {
    ...
    #if ENABLED(USE_Z_SENSORLESS)
       stepperZ.anker_homing_threshold(use_z_sensorless.an_stall_value);
       #ifdef ANKER_Z2_STALL_SENSITIVITY
        stepperZ2.anker_homing_threshold(use_z_sensorless.an_stall_value);
       #endif
    #endif
  ...
  void homeaxis(const AxisEnum axis) {
    ...
    #else
      TERN_(USE_Z_SENSORLESS, if(axis == Z_AXIS){anker_homing.set_first_end_z_axis(Z_AXIS_IDLE);});
    #endif
    ...
    if (Z_AXIS == axis){planner.synchronize();}
  #endif
    ...
    if (bump) {
      ...
      do_homing_move(axis, rebump, (axis == Z_AXIS ? 0 : get_homing_bump_feedrate(axis)), true);
    ...
    if (axis == Z_AXIS)
      {
        ...
        #if ENABLED(ANKER_SIMPLE_HOMING)
          const xyz_float_t endstop_backoff = HOMING_BACKOFF_POST_MM;
          if (endstop_backoff[Z_AXIS]) {
            current_position[Z_AXIS] -= ABS(endstop_backoff[Z_AXIS]) * axis_home_dir;
            line_to_current_position(homing_feedrate(Z_AXIS));
            planner.synchronize();
          }
          TERN_(ANKER_ANLIGN, anker_align.run_align());
        #else
  ...
  int Probe_homeaxis(const AxisEnum axis,uint8_t anker_homing) {
    ...
    TERN_(ANKER_FIX_ENDSTOPR, endstops.set_anker_endstop(3));
    ...
    TERN_(ADAPT_DETACHED_NOZZLE, uart_nozzle_tx_point_type(POINT_G28, 1));
    ...
    MYSERIAL2.printLine("Probe_homeaxis Home Fast: =%3.5f\n", probe_first);
    ...
    if (bump) {
      ...
      if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPAIR("probe_Move Away: ", -bump, "mm");
       anker_do_probe_homing_move(axis, -bump, TERN(HOMING_Z_WITH_PROBE, (axis == Z_AXIS ? MMM_TO_MMS(HOMING_RISE_SPEED) : 0), 0), false);
        ...
        #if ENABLED(BLTOUCH_SLOW_MODE)
          if (axis == Z_AXIS && bltouch.deploy()) return; // Intermediate DEPLOY (in LOW SPEED MODE)
        #elif ENABLED(HOMING_Z_WITH_PROBE)
          if (axis == Z_AXIS && probe.anker_deploy()) return 2;
        #endif
      #endif

      // Slow move towards endstop until triggered
      const float rebump = bump * 3;
      if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPAIR("probe_Re-bump: ", rebump, "mm");
      
      #if ENABLED(ANKER_PROBE_DETECT_TIMES)
        if (axis == Z_AXIS){
          uint8_t count_flag = 0; // The maximum number of consecutive attempts
          uint8_t try_again = 0;
          uint8_t Detecting_effectiveness = false;
          //const xy_pos_t move_away[4] = {{-1.0, -1.0}, {2.0, 0.0}, {0.0, 2.0}, {-2.0, 0.0}};
          safe_delay(200);
          for(try_again = 0; try_again < 4; try_again++)
          {
            while(count_flag <= 2)// The maximum number of consecutive attempts = 2
            {
              TERN_(ADAPT_DETACHED_NOZZLE, uart_nozzle_tx_point_type(POINT_G28, (uint8_t)(2+count_flag)));
              TERN_(ANKER_PROBE_SET, anker_probe_set.probe_start(anker_probe_set.leveing_value));

              if (anker_do_probe_homing_move(axis, rebump, get_homing_bump_feedrate(axis), true)) {// timer out!!! move up and try again
                MYSERIAL2.printLine("echo: Probe_homeaxis-timer_out=%3.5f, move up and try again\r\n", planner.triggered_position_mm(_AXIS(Z)));
                anker_do_probe_homing_move(axis, -bump, TERN(HOMING_Z_WITH_PROBE, (axis == Z_AXIS ? MMM_TO_MMS(HOMING_RISE_SPEED) : 0), 0), false);
                continue;
              }
              float probe_second =  planner.triggered_position_mm(_AXIS(Z));
              MYSERIAL2.printLine("echo: Z homing try:%d Probe Z at:%3.5f last:%3.5f diff:%3.5f\r\n", count_flag, probe_second, probe_first, (probe_first - probe_second));
              
              if(ABS(probe_first - probe_second) < Z_PROBE_DETECTION_DEVIATION)
                {Detecting_effectiveness = true; break;} // OK!
              else{ // move up and try again
                if(count_flag != 0 && ABS(probe_first - probe_second) < 0.3f){// Filter out the first point
                  // Return a weighted average of the first and second probes
                  const float measured_z = (probe_second * 3.0f + probe_first * 2.0f) * 0.2f;
                  probe_first = measured_z;
                  safe_delay(5);
                  MYSERIAL2.printLine("echo: Z homing-weighted_average:%3.5f\r\n", measured_z);
                }else{
            ...
            if(true == Detecting_effectiveness)
            {// OK
                break;
            }
            else{ // Slightly move the X/Y axis and try again.
              xy_pos_t dest;
              ...
              probe_first = planner.triggered_position_mm(_AXIS(Z));
              anker_do_probe_homing_move(axis, -bump, TERN(HOMING_Z_WITH_PROBE, (axis == Z_AXIS ? MMM_TO_MMS(HOMING_RISE_SPEED) : 0), 0), false);
            }
          }

          if(count_flag >= 3 || try_again >= 4) {
            MYSERIAL2.printLine("echo: Probe_homeaxis-attempts_err=%3.5f\r\n", planner.triggered_position_mm(_AXIS(Z)));
            error_status = 0;
          } // error!!! try homing again
        }else{
      ...
      if((anker_probe_set.run_step==1)&&(anker_homing==2))
      {
         anker_probe_set.home_delay();
      }
  ...
  void homeaxis(const AxisEnum axis) {
    ...
    // Set flags for X, Y, Z motor locking
      ...
      else if(axis == Z_AXIS)
      {
        endstops.set_anker_endstop(2);
        #if ENABLED(ANKER_PROBE_SET)
         anker_probe_set.probe_start(anker_probe_set.homing_value);
        #endif
      }
    #endif
    //
...
void set_axis_is_at_home(const AxisEnum axis) {
   ...
   */
    ...
    if (axis == Z_AXIS) {
      #if HOMING_Z_WITH_PROBE
        MYSERIAL1.printf("1-current_position.z:%f\r\n",current_position.z);
        MYSERIAL1.printf("1-probe.offset.z:%f\r\n",probe.offset.z);
        current_position.z -= probe.offset.z;
        MYSERIAL1.printf("2-current_position.z:%f\r\n",current_position.z);
        MYSERIAL1.printf("2-probe.offset.z:%f\r\n",probe.offset.z);
