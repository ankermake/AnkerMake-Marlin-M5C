...
typedef struct {
  #define FL_LINE_NUM 4 //
  #define CUR_LINE    0 // current line
  #define PREV_LINE   1 // previous line
  #define PPREV_LINE  2
  #define PREV3_LINE  3
  #define CURVITY_2LINE 0
  #define CURVITY_3LINE 1
  #define CURVITY_4LINE 2
  #define CURVITY_5LINE 3
  #define COSTEHTA_35DEG (-0.819152f)
  #define CD_MAXIMUM_RESOLUTION (5.0f) // maximum Resolution * 80%
  #define IS_SMOOTH_LINE(point, deg) ((point.cos_theta[CUR_LINE] < deg) && (point.cos_theta[PREV_LINE] < deg))
  float_t cos_theta[FL_LINE_NUM];
  xy_float_t vec[FL_LINE_NUM];
  double radius[CURVITY_5LINE+1];
  float pre_millimeters;
}  corner_calculation_t;
#endif

#if ENABLED(ANKER_VIBRATION_CONTROL)
typedef struct {
  #define VC_CLOSED   0
  #define VC_TS_CURVE  1
  #define VC_ZEROCONF 2
  uint8_t mode; // M4897 S - 0=CLOSED 1=T/S CURE 2= Zeroconf
  struct {
    float N,     // (mm/s) Zeroconf M4897 T - N = 1,2,3...
          omiga; // (mm/s) Zeroconf M4897 O - omiga =380
  }Zeroconf;
...
class Planner {
  public:
    ...
    static float steps_to_mm[DISTINCT_AXES];          // Millimeters per step
      ...
      TERN_(ANKER_RETRACTION_E_JERK, static float retraction_e_jerk);
    #endif
      ...
      #endif
      static volatile uint8_t LIN_ADV_version_change; // = 1 version2 =0 default
    #endif
    ...
    static skew_factor_t skew_factor;
      ...
      static float_t xy_freq_limit_mm;
    ...
    #if ENABLED(ANKER_CORNER_CALC)
      static corner_calculation_t corner;
    #endif

    #if ENABLED(ANKER_VIBRATION_CONTROL)
      static vibration_control_t VC;
      static float anker_start_accel(block_t * const block, const uint32_t esteps);
    #endif
  ...
  public:
    ...
    // For an axis set the Maximum Jerk (instant change) in mm/s
      ...
      TERN_(ANKER_RETRACTION_E_JERK, static void set_max_e_jerk_for_retract_acceleration(float inMaxJerkMMS));
    #else
    ...
    // Apply fan speeds
       ...
       */
        ...
        static inline float fade_scaling_factor_for_z(const_float_t rz) {
          ...
          if (!z_fade_height || rz <= (0.4)) return 1;
  ...
  public:
    ...
    static void synchronize();
      ...
      static void anker_z_offset_synchronize();
    #endif

    #if ENABLED(ANKER_BELT_CHECK)
      static void anker_belt_check_synchronize(uint8_t idex);
    #endif
  ...
  private:
    ...
    #if HAS_JUNCTION_DEVIATION || ENABLED(ANKER_CORNER_CALC)
    ...
    #endif // !CLASSIC_JERK
    #if ENABLED(ANKER_CORNER_CALC)
      static void costheta_calc(const uint8_t moves_queued, const abce_long_t target, const abce_float_t steps_dist_mm, const uint32_t esteps, const float inverse_millimeters);
    #endif
