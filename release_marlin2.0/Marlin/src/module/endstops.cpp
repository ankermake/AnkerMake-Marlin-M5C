...
Endstops::endstop_mask_t Endstops::live_state = 0;
 ...
 volatile uint8_t Endstops::homing_filter_count = 0;// Called at ~1KHz from Temperature ISR
#endif
...
void Endstops::not_homing() {
  ...
  void Endstops::validate_homing_move(const AxisEnum axis) {
    if (trigger_state()) hit_on_purpose();
      ...
      #ifdef ADAPT_DETACHED_NOZZLE
      uart_nozzle_tx_notify_error();
      #endif
      switch(axis)
      {
        case X_AXIS:
          MYSERIAL2.printLine("Error:Homing Error X_AXIS\r\n");
          break;
        case Y_AXIS:
          MYSERIAL2.printLine("Error:Homing Error Y_AXIS\r\n");
          break;
        case Z_AXIS:
      ...
      MYSERIAL2.printLine("echo: validate_homing_move-trigger_err\r\n");
...
void Endstops::update() {
  ...
  #define PROCESS_ENDSTOP(AXIS, MINMAX) do { \
    if (TEST_ENDSTOP(_ENDSTOP(AXIS, MINMAX)) && (!homing_filter_count)) { \
      _ENDSTOP_HIT(AXIS, MINMAX); \
      planner.endstop_triggered(_AXIS(AXIS)); \
      anker_homing.xy_triger();\
    } \
  }while(0)
  ...
  // Call the endstop triggered routine for single endstops
    ...
    const byte dual_hit = TEST_ENDSTOP(_ENDSTOP(A, MINMAX)) | (TEST_ENDSTOP(_ENDSTOP(A##2, MINMAX)) << 1); \
    if (dual_hit && (!homing_filter_count)) { \
      _ENDSTOP_HIT(A, MINMAX);\
      if (TEST_ENDSTOP(_ENDSTOP(A, MINMAX))) { anker_homing.set_first_end_z_axis(Z_AXIS_IS_Z1);stepper.set_z1_lock(true);}\
      if (TEST_ENDSTOP(_ENDSTOP(A##2, MINMAX))) { anker_homing.set_first_end_z_axis(Z_AXIS_IS_Z2);stepper.set_z2_lock(true);}\
      /* if both endstops were triggered during homing... */ \
      if (!stepper.separate_multi_axis || 0b11 == stepper.get_z_lock()) {\
        planner.endstop_triggered(_AXIS(A));stepper.set_all_z_lock(false);\
