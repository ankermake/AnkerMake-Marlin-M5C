...
#include "../../core/debug_out.h"
 ...
 #include "../../feature/anker/anker_homing.h"
    ...
    if (position_is_reachable(destination)) {
      ...
      TERN_(SENSORLESS_HOMING, safe_delay(500)); // Short delay needed to settle
        ...
        if(anker_homing.is_home_z) 
        {
          ...
          #ifndef NO_DUAL_Z
          #if ENABLED(USE_Z_SENSORLESS) && (defined(Z_CURRENT_HOME) && Z_CURRENT_HOME != Z_CURRENT)
              const uint16_t ltmc_save_current_Z = stepperZ.getMilliamps();
              stepperZ.rms_current(Z_STALL_CURRENT);
              const uint16_t ltmc_save_current_Z2 = stepperZ2.getMilliamps();
  ...
  inline void anker_home_z_safely() {
    ...
    if (position_is_reachable(destination)) {
      ...
      if(thermalManager.degHotend(0)>=EXTRUDE_MINTEMP && anker_align.g36_running_flag == true)
      {
        anker_homing.is_clean=true;
        #ifndef NO_DUAL_Z
          #if ENABLED(USE_Z_SENSORLESS) && (defined(Z_CURRENT_HOME) && Z_CURRENT_HOME != Z_CURRENT)
            const uint16_t tmc_save_current_Z = stepperZ.getMilliamps();
            stepperZ.rms_current(Z_STALL_CURRENT);
            const uint16_t tmc_save_current_Z2 = stepperZ2.getMilliamps();
            stepperZ2.rms_current(Z2_STALL_CURRENT);
            TERN_(SENSORLESS_STALLGUARD_DELAY, safe_delay(SENSORLESS_STALLGUARD_DELAY));// Short delay needed to settle
          #endif
          homeaxis(Z_AXIS);
         ...
         do_blocking_move_to_xy(destination, MMM_TO_MMS(2100));
  ...
  motion_state_t begin_slow_homing() {
    ...
    planner.settings.max_acceleration_mm_per_s2[X_AXIS] = 200;
...
void GcodeSuite::G28() {
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    TERN_(IMPROVE_HOMING_RELIABILITY, end_slow_homing(saved_motion_state));
      ...
      #if HAS_CURRENT_HOME(X)
        stepperX.rms_current(tmc_save_current_X);//In order to coordinate with the Z-axis homing operation, set the X/Y axis in the moving state.
      #endif
      #if HAS_CURRENT_HOME(Y)
  ...
  #if ENABLED(EVT_HOMING_5X)
    anker_homing.after_homing_action();
   #endif
