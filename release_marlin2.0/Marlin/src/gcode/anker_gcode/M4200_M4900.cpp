...
void GcodeSuite::M4201() {
  ...
  LOOP_LOGICAL_AXES(i) {
    if (parser.seenval(axis_codes[i])) {
      ...
      constexpr xyze_float_t max_accel_edit = LA_MAX_ACCEL_EDIT_VALUES;
...
/**
 * M4897: T/S curve switching and Zero configuration(Zeroconf)
 *
 * S: 0=CLOSED 1=T/S CURE 2= Zeroconf
 * T: planner.VC.N
 * O: planner.VC.omiga
 */
void GcodeSuite::M4897(){

  if (parser.seenval('S')){ // S: 0=CLOSED 1=T/S CURE 2= Zeroconf
    uint8_t mode = parser.value_byte();
    planner.VC.mode = mode;
    MYSERIAL2.printLine("VC.mode = %d\n", planner.VC.mode);
  }

  if (parser.seenval('T')){ // T: planner.VC.N
      uint8_t cycle = parser.value_byte();
      if (cycle > 0){planner.VC.Zeroconf.N = cycle;}
      MYSERIAL2.printLine("VC.Zeroconf.N = %d\n", cycle);
  }

  if (parser.seenval('O')) { // O: planner.VC.omiga
    const float omiga = parser.value_float();
    if (WITHIN(omiga, 0.1f, 10000.0f)) {planner.VC.Zeroconf.omiga = omiga;}
    MYSERIAL2.printLine("VC.Zeroconf.omiga = %3.5f\n", omiga);
  }
}
...
/**
 * M4898: Set motion debug control
 *
 * S: 0=CLOSED 1=OPEN
 * T: 0=NONE_TRACK, 1=POS_TRACK, 2=PLR_TRACK
 */
void GcodeSuite::M4898(){

    if (parser.seenval('T')){
       uint8_t mode = parser.value_byte();
       motion_track.mode(mode);
       MYSERIAL2.printLine("echo:motion_track.mode = %d\n", mode);
    }

    if (parser.seenval('S')){
       uint8_t switch_flag = parser.value_byte();
       motion_track.contorl(!!switch_flag);
      MYSERIAL2.printLine("echo:motion_track.switch = %d\n", !!switch_flag);
    }
}
#endif
        ...
        case LIN_ADV_VERSION_0:
          {
            ...
            TERN_(SENSORLESS_STALLGUARD_DELAY, safe_delay(SENSORLESS_STALLGUARD_DELAY));// Short delay needed to settle
            gcode.process_subcommands_now_P(PSTR("M913 X329 Y329\n"));
            TERN_(SENSORLESS_STALLGUARD_DELAY, safe_delay(SENSORLESS_STALLGUARD_DELAY));// Short delay needed to settle
          }
          break; // old version
        ...
        case LIN_ADV_VERSION_2: // new version only new K lin_adv
          {
            ...
            TERN_(SENSORLESS_STALLGUARD_DELAY, safe_delay(SENSORLESS_STALLGUARD_DELAY));// Short delay needed to settle
            gcode.process_subcommands_now_P(PSTR("M913 X0 Y0\n"));
            TERN_(SENSORLESS_STALLGUARD_DELAY, safe_delay(SENSORLESS_STALLGUARD_DELAY));// Short delay needed to settle
          }
          break; // new version
        ...
        case LIN_ADV_VERSION_3: // new version Scurve + new K lin_adv
          {
            ...
            planner.settings.travel_acceleration = planner.settings.acceleration = LA_V1_DEFAULT_ACCELERATION;
            planner.settings.retract_acceleration = LA_V1_DEFAULT_RETRACT_ACCELERATION;
            set_max_jerk(X_AXIS, LA_V1_DEFAULT_XJERK);
            set_max_jerk(Y_AXIS, LA_V1_DEFAULT_YJERK);
            set_max_jerk(Z_AXIS, LA_V1_DEFAULT_ZJERK);
            set_max_jerk(E_AXIS, LA_V1_DEFAULT_EJERK);
            const feedRate_t feedrate[DISTINCT_AXES] = LA_V1_DEFAULT_MAX_FEEDRATE;
            planner.settings.max_feedrate_mm_s[X_AXIS] = feedrate[X_AXIS];
            planner.settings.max_feedrate_mm_s[Y_AXIS] = feedrate[Y_AXIS];
