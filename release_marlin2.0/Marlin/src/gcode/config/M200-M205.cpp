  ...
  if (!parser.seen("PRST")) {
    ...
    // 'S' for legacy compatibility. Should NOT BE USED for new development
      ...
      if (parser.seenval('S'))
      {
         ...
         if(WITHIN(acc_temp, 1, default_accel))
         {
      ...
      {
         acc_temp=parser.value_linear_units();
         if(WITHIN(acc_temp, 1, default_retract))
         {
      ...
      {
         acc_temp=parser.value_linear_units();
         if(WITHIN(acc_temp, 1, default_travel))
         {
...
void GcodeSuite::M205() {
  ...
  if (parser.seenval('T')) planner.settings.min_travel_feedrate_mm_s = parser.value_linear_units();
    ...
    TERN_(ANKER_RETRACTION_E_JERK, if (parser.seenval('R')) planner.set_max_e_jerk_for_retract_acceleration(parser.value_linear_units()));
